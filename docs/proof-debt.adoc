...
Per https://github.com/hyperpolymath/standards/blob/main/docs/TRUSTED-BASE-REDUCTION-POLICY.adoc[`hyperpolymath/standards` — Trusted-Base Reduction Policy].
...
. *`executePreservesTypes` proves nothing.* Its statement reduces to `… → True`, with
...
* (none yet — entries are removed from §(d) and _not_ listed here once a proof lands)
...
coverage: `lake test` reports `no test driver configured` … adding a `@++[++test++_++driver++]++` is a
...
So items _can_ now be moved here — but none has been yet, and none should be moved without
...
|`src/GqlDt/IR.lean:721` |`exampleInsertIR` — "Simplified to use axioms to avoid complex Prompores proof obligations"
...
** *Kind*: the one item with a _reasoned_ justification in-source — the dynamic schema
...
§(c), because it _is_ derivable in principle. Not yet §(b) either — but the reason has
...
|`src/GqlDt/TypeInference.lean:179` |`{vbar} .admit =++>++ "sorry"` — a *string literal* returned by a pretty-printer
