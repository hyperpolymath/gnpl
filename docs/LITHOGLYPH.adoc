...
* `lake build` green — **partially satisfied**: it builds, 34/35 targets, and Lean reports
  no incomplete proofs. But Lean's `sorry` warning does not fire on `axiom`, and GQLdt
  declares **16 axioms**, all stubs — five of them in *executable* position, so those
  functions have no implementation at all. See `docs/proof-debt.md`. Steps 1–3 of the
