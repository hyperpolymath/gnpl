== GQL-DT Language Design - Implementation Status

*SPDX-License-Identifier:* CC-BY-SA-4.0
*SPDX-FileCopyrightText:* 2026 Jonathan D.A. Jewell (@hyperpolymath)

*Date:* 2026-02-01
*Status:* ⚠️ PARTIAL — selected typed-value codecs exist; complete IR encoding and decoding are not implemented
...
|`Prompores` |6 dimensions {plus} overall |Auto-computed |✅ Complete
...
*Status:* Partial. Selected typed-value codecs only; this support explicitly excludes complete IR encoding and decoding. The pipeline refuses complete IR interchange when it would lose clauses or require unchecked proof reconstruction. The supported public snapshot format is documented in link:narration-slice.adoc[the narration guide]; link:executable-boundary.adoc[the executable boundary] supersedes the historical codec claims below.

*Files:*

* `src/GqlDt/Serialization.lean` - Selected typed-value codecs only; no complete IR encoder or decoder
* `src/GqlDt/IR.lean` - Typed IR definitions; not completely encoded or decoded by the codecs
...
===== CBOR (RFC 8949) - Selected Typed Values
...
*Features for selected typed values only:*
...
Typed IR (preserves dependent types)
    ↓ Planned complete IR encoding (not implemented)
...
|*3. Type Safety* |✅ COMPLETE |5 files |Compile-time {plus} runtime enforcement
|*4. Serialization* |⚠️ PARTIAL |1 file |Selected typed-value codecs only; no complete IR encoding or decoding
...
*Current:* Type system, grammar, and type safety are complete. Serialization supports selected typed-value codecs only; complete IR encoding and decoding are not implemented.

*Ready to implement:*

[arabic]
. ✅ IR data structures → *DONE* (src/GqlDt/IR.lean)
. ✅ Type inference → *DONE* (src/GqlDt/TypeInference.lean)
. ⚠️ Selected typed-value codecs → *PARTIAL* (src/GqlDt/Serialization.lean); excludes complete IR encoding and decoding
. ⏳ Actual parser (text → AST) - NEXT
. ⏳ AST → IR generation - NEXT
. ⏳ Complete IR encoding and decoding, including CBOR transport - NEXT

*Language design implementation: ⚠️ PARTIAL — complete IR encoding and decoding remain outstanding*

'''''

*Document Status:* Implementation status with serialization gap recorded

*Recommendation:* Proceed with parser implementation (M6) while treating complete IR encoding and decoding as outstanding work.
